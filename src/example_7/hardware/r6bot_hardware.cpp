...
RobotSystem::RobotSystem() : logger_(rclcpp::get_logger("DeliRasp")) {}

CallbackReturn RobotSystem::on_init(const hardware_interface::HardwareInfo & info)
{ 
  RCLCPP_INFO(logger_, "RobotSystem::on_init...");
  // hardware_interface::HardwareInfo &info : r6bot.ros2_control.xacro information
...
{
  RCLCPP_INFO(logger_, "RobotSystem::on_configure...");
  // reset values always when configuring hardware
  RCLCPP_INFO(logger_, "joint_state_interfaces_ contents:");
  for (const auto & [name, descr] : joint_state_interfaces_)
  {
    RCLCPP_INFO(logger_, "  Interface Name: %s", name.c_str());
    set_state(name, 0.0);
  }
  RCLCPP_INFO(logger_, "joint_command_interfaces_ contents:");

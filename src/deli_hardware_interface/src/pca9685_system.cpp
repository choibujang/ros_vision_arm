#include "deli_hardware_interface/pca9685_system.hpp"
  ...
  const hardware_interface::HardwareInfo & info)
{
  ...
  pca.set_pwm_freq(50.0); // 모터 드라이버 제어를 위해 주파수 50으로 맞춤
  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  for (auto i = 0u; i < info_.joints.size(); i++)
  {
    if (std::isnan(pos_commands_[i]))
    {
      pos_commands_[i] = 0;
    }

    if (std::isnan(vel_commands_[i]))
    {
      vel_commands_[i] = 0;
    }
  }
...
PLUGINLIB_EXPORT_CLASS(
  deli_hardware_interface::Pca9685SystemHardware, hardware_interface::SystemInterface)

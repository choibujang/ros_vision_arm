...
int main(int argc, char **argv) try {
    ...
    std::shared_ptr<ob::StreamProfileList> profileList = pipe.getStreamProfileList(OB_SENSOR_COLOR);
    for (int i = 0; i < profileList->count(); i++) {
        auto profile = profileList->getProfile(i)->as<ob::VideoStreamProfile>();
        std::cout << "Supported Profile: "
                << profile->width() << "x" << profile->height()
                << " " << profile->fps() << " FPS, Format: " << profile->format() << std::endl;
    }

    // std::shared_ptr<ob::StreamProfileList> profileList = pipe.getStreamProfileList(OB_STREAM_COLOR);
    ...
    // if (!selectedProfile) {
    //     std::cerr << "Warning: Requested profile not found! Using default profile." << std::endl;
    //     selectedProfile = profileList->getProfile(0)->as<ob::VideoStreamProfile>();
    // }
